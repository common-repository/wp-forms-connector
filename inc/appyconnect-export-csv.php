...
class AppyConnect_Expoert_CSV{
    ...
    public function download_csv_file(){
        ...
        if( isset($_REQUEST['csv']) && isset( $_REQUEST['nonce'] ) ){
            ...
            $heading_row = $appyconnector->get_results("SELECT form_id, form_value, form_date FROM $table_name
            ...
            $total_rows  = $appyconnector->get_var("SELECT COUNT(*) FROM $table_name WHERE form_post_id = '$fid' "); 
            $per_query    = 1000;
            $total_query  = ( $total_rows / $per_query );

            $this->download_send_headers( "connect-" . date("Y-m-d") . ".csv" );
            ...
            for( $p = 0; $p <= $total_query; $p++ ){
                ...
                $offset  = $p * $per_query;
                $results = $appyconnector->get_results("SELECT form_id, form_value, form_date FROM $table_name
                ...
                foreach ($results as $result) :
                    ...
                    foreach ($resultTmp as $key => $value):
                        ...
                        if (strpos($key, 'connect_file') !== false ){
                            $data[$key][$i] = $connect_dir_url.'/'.$value;
                            continue;
                        }
                        if ( is_array($value) ){
